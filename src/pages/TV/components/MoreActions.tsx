...
const MoreActions = ({ record, reloadData }: any) => {
  ...
  const delBanner = async () => {
    const { response, error } = await api.put<any, any>(
      `/api/v1/manager/tv/${slug}/del-banner`,
    );
    if (error) {
  ...
  const setBanner = async () => {
    const { response, error } = await api.put<any, any>(
      `/api/v1/manager/tv/${slug}/set-banner`,
    );
  ...
  const handleDelete = async () => {
    const { error } = await api.delete<any, any>(`/api/v1/manager/tv/${slug}`);
  ...
  const handleMenuClick = ({ key }: any) => {
    switch (key) {
      case 'update':
        openModal('UpdateTVModalForm', { slug });
        break;
      case 'delete':
