...
import CreateResourceModalForm from './CreateResourceModalForm';
...
const ResourcesModal = ({ modalName = 'ResourcesModal' }: Props) => {
  ...
  const {
    ...
    open && params?.slug ? `/api/v1/manager/tv/${params.slug}` : null,
    api.get,
  );
  ...
  const handleSendMessage = (record: any) => {
    ...
    const params: Record<string, any> = {
      title: `${data.name}${year}${record.remark || ''}  `,
      ...
      keywords: keywords
        ? `标签：#${data.name} ${keywords} ${isIncludeREMUX ? '#REMUX' : ''}  `
        : '',
      // TODO FIXME
      web_link: `<a href="https://hdhive.org/${data.record_type}/${data.slug}">${data.name} - 影巢</a>  `,
    };
